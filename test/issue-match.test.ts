...
test('one candidate that satisfies every condition is the match', () => {
  const hit = matchIssue([VENOM_250], { matchKey: 'venom', number: '250', coverYear: 2025 })
  expect(hit?.id).toBe(1)
})

test('no candidate at all is no match', () => {
  expect(matchIssue([], { matchKey: 'venom', number: '250', coverYear: 2025 })).toBeNull()
})
...
test('a scraped year one behind the cover year still matches', () => {
  const hit = matchIssue([row(1, 'Venom #251 (2025)', '251', 2025)], { matchKey: 'venom', number: '251', coverYear: 2026 })
  expect(hit?.id).toBe(1)
})

test('a scraped year one ahead of the cover year still matches', () => {
  const hit = matchIssue([row(1, 'Venom #251 (2026)', '251', 2026)], { matchKey: 'venom', number: '251', coverYear: 2025 })
  expect(hit?.id).toBe(1)
})

test('two years apart is too far to be the same issue', () => {
  expect(matchIssue([row(1, 'Venom #251 (2023)', '251', 2023)], { matchKey: 'venom', number: '251', coverYear: 2025 })).toBeNull()
})
...
test('two candidates within the window are no match, not a guess', () => {
  const hits = matchIssue(
    ...
    { matchKey: 'venom', number: '003', coverYear: 2017 },
  )
  expect(hits).toBeNull()
})

test('a different series with the same number does not match', () => {
  expect(matchIssue([row(1, 'All-New Venom #1 (2025)', '001', 2025)], { matchKey: 'venom', number: '001', coverYear: 2025 })).toBeNull()
})

test('a leading "The" does not separate a series from itself', () => {
  const hit = matchIssue([row(1, 'The Mighty Thor #700 (2017)', '700', 2017)], { matchKey: 'mighty thor', number: '700', coverYear: 2017 })
  expect(hit?.id).toBe(1)
})

// A post holding #1-85 is not issue #1, however its number parses.
test('a bundle of many issues does not match a single issue', () => {
  expect(matchIssue([row(1, 'Venom #1 – 85 (2025)', '001', 2025)], { matchKey: 'venom', number: '001', coverYear: 2025 })).toBeNull()
})

test('a collected edition does not match a single issue', () => {
  expect(matchIssue([row(1, 'Venom Vol. 1 (TPB) (2025)', '001', 2025)], { matchKey: 'venom', number: '001', coverYear: 2025 })).toBeNull()
})

test('a row with no year can never be matched', () => {
  expect(matchIssue([row(1, 'Venom #250 (2025)', '250', null)], { matchKey: 'venom', number: '250', coverYear: 2025 })).toBeNull()
})

test('an issue with no cover year can never be matched', () => {
  expect(matchIssue([VENOM_250], { matchKey: 'venom', number: '250', coverYear: null })).toBeNull()
})

test('a row with no number can never be matched', () => {
  expect(matchIssue([row(1, 'Venom (2025)', null, 2025)], { matchKey: 'venom', number: '250', coverYear: 2025 })).toBeNull()
})
...
test('the one match is found among candidates that do not qualify', () => {
  const hit = matchIssue(
    ...
    { matchKey: 'venom', number: '250', coverYear: 2025 },
  )
  expect(hit?.id).toBe(1)
